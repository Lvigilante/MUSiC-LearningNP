        ...
        if(Scan==False):
            for g in SamDir:
                if "grid" in g:
                    ...
                    #print(f + "/" + g)
                    #for l in GridDir:
                    if "MusicOutDir.tar.gz" in GridDir and "AnalysisOutput" in GridDir:
                        #print(colored(" Both OutDir and AnalysisOutput in grid Folder: ", "yellow"))
                        #print(InDir + "/" + f + "/" + m + "/" + g)
                        n1= n1 +1
                    elif "MusicOutDir.tar.gz" in GridDir and "AnalysisOutput" not in GridDir:
                        excmd=""
                        prpath= InDir + "/" + f + "/" + m + "/" + g + "/" 
                        print(colored("Only MusicOutDir  in grid Folder:", "green")) 
                        print(InDir + "/" + f + "/" + m + "/" + g)
                        n2= n2 +1
                        #excmd= "tar xvzf " + prpath + "MusicOutDir.tar.gz" + " -C " + prpath
                        #print(excmd)
                        os.system(excmd)
                    elif "AnalysisOutput" in GridDir and "MusicOutDir.tar.gz" not in GridDir :
                        #print(colored("Only AnalysisOutput  in grid Folder:", "yellow")) 
                        #print(InDir + "/" + f + "/" + m + "/" + g)
                        n1= n1 +1
                    else:
                        print(colored(" No Output or OutDir in grid Folder:", "red")) 
                        print(InDir + "/" + f + "/" + m + "/" + g)
                        n3= n3 +1
        else:
            if "grid" in m:
